...
public class ResourceController {
    ...
    public ResponseEntity<ApiResponse<List<ResourceResponseDTO>>> getVersionHistory(
            @PathVariable Long id,
            HttpServletRequest request) {
        Long userId = getCurrentUserId(request);
        List<ResourceResponseDTO> versions = resourceService.getVersionHistory(id, userId);
        return ResponseEntity.ok(ApiResponse.success(versions));
    }

    // POST /api/resources/{id}/rollback - 回滚到指定版本

    @PostMapping("/{id}/rollback")
    public ResponseEntity<ApiResponse<ResourceResponseDTO>> rollbackToVersion(
            @PathVariable Long id,
            @RequestBody Map<String, Integer> body,
            HttpServletRequest request) {
        Long userId = getCurrentUserId(request);
        Integer targetVersionNo = body.get("targetVersionNo");
        if (targetVersionNo == null) {
            return ResponseEntity.badRequest()
                    .body(ApiResponse.error("targetVersionNo is required"));
        }
        ResourceResponseDTO resource = resourceService.rollbackToVersion(id, userId, targetVersionNo);
        return ResponseEntity.ok(ApiResponse.success("Rollback successful", resource));
    }
